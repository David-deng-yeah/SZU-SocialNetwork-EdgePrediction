...
class elm():
    ...
    * Weight initialization method
    ...
    def __init__(self, hidden_units, activation_function,  x, y, C, elm_type, one_hot=True, random_type='normal'):
        ...
        # we transform the label into one-hot code,whose aims is to suit the outputlayer
    ...
    * time of computing beta
    ...
    def predict(self, x):
        ...
        # 直接做softmax然后输出
        # self.y_ = np.exp(self.y_)/np.sum(np.exp(self.y_), axis=1).reshape(-1, 1)
        return self.y_
    '''
    就是输入x，输出y的softmax形式
    '''
